...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        def drs_schemes():
            ...
            gStep_ex_sus = (
                exsus_end_float - exsus_start_float
            ...
            if gPlotdata:
                if len(gSchemes) != 0:
                    try:
                        effsstsPlot_DRS.effsstsPlotAll(
                            gPrefixdata,
                            gPlotall,
                            gSchemes,
                            gSLenMinValue,
            ...
            if gmpCheck:
                if len(gSchemes) != 0:
                    try:
                        effsstsPlot_DRS.effsstsPlotAllmulti(
                            gPrefixdata,
                            gPlotall,
                            gmultiplot,
                            garwrap,
...
def drs_task():
    ...
    i = 0
    for gEx, gEx_Sus in zip(
        np.linspace(gExStart, gExEnd, num=gGran),
        np.linspace(gEx_Sus, gEx_SusEnd, num=gGran),
    ):
        tasksets_for_config = []
        for _ in range(0, gNumberOfTaskSets):
            tasks = drstask.taskGeneration_drs(
                gNumberOfTasksPerSet, gEx_Sus, gEx, Pmin=100, numLog=1
            )
            print(tasks)

...
def GMFPA(tasks):
    ...
    Tb = m.addMVar(     (len_tasks,H,len_segs,len_segs) ,vtype=GRB.CONTINUOUS   ,name="Tb")
    Xitjk = m.addMVar(  (len_tasks,H,len_segs,len_segs) ,vtype=GRB.BINARY       ,name="Xitjk")
    Yitjk = m.addMVar(  (len_tasks,H,len_segs,len_segs) ,vtype=GRB.CONTINUOUS   ,name="Yitjk")
    Yitj = m.addMVar(   (len_tasks,H,len_segs)          ,vtype=GRB.CONTINUOUS   ,name="Yitj")
    Yit = m.addMVar(    (len_tasks,H)                   ,vtype=GRB.CONTINUOUS   ,name="Yit")
    ...
    # Condition 7           
    m.addConstrs((Yitjk[i,t,j,k] == Xitjk[i,t,j,k] * Eik[i][k] + math.floor(t/Pi[i])*Eik[i][k] 
                            for t in range(H)
                            for i in range(len_tasks)
                            for j in range(len_segs)
                            for k in range(len_segs)),name='c7')
    # Condition 8
    m.addConstrs(((t-Tb[i,t,k,j])/Pi[i] <= Xitjk[i,t,j,k] - realmin/Pi[i]
                            for t in range(H)
                            for i in range(len_tasks)
                            for j in range(len_segs)
                            for k in range(len_segs)),name='c8')
    # Condition 9
    m.addConstrs((Tb[i,t,k,j] == Dik[i,j:k].sum()+Dik[i,k]+math.floor(t/Pi[i])*Pi[i]
                            for t in range(H)
                            for i in range(len_tasks)
                            for j in range(len_segs)
                            for k in range(len_segs)),name='c9')
    #Sum for condition 9
    # m.addConstrs((Dijk[i,j,k] == Dik[i,j:k].sum()
    #                         for i in range(len_tasks)
    #                         for j in range(len_segs)
    #                         for k in range(len_segs))
    #                         ,name='c9')
    ...
    # Condition 12
    m.addConstrs((Yit[:,t].sum() <= L*t
                            for t in range(H)),name='c12')

...
def Wnc(i, l, xil, slj, tasks):
    ...
    if i != l:
        val = min(
            math.floor(xil / tasks[i]["period"]) * tasks[i]["execution"],
            xil - tasks[l]["execution"] - slj + 1,
        )
    else:
        val = min(
            math.floor(xil / tasks[l]["period"]) * tasks[l]["execution"]
            - tasks[l]["execution"],
            xil - tasks[l]["period"],
        )
    return val
...
def Wc(i, l, xil, slj, tasks):
    ...
    if i != l:
        ...
        val = min(
            Delta(l, xil, tasks) - tasks[l]["execution"], xil - tasks[l]["period"]
        )
    return val
...
def Delta(i, t, tasks):
    ...
    val += min(
        tasks[i]["execution"],
        t - math.ceil(t / tasks[i]["period"]) * tasks[i]["period"] + tasks[i]["period"],
    )
    return val
...
# Note: if total utilization is close to 100 percent, this causes problems, since upperbound explodes.
def WLAEDF(tasks):
    ...
    for l in range(len(tasks)):
        ...
        for slj in range(int(tasks[l]["sslength"]) + 1):
            ...
            upperbound = math.ceil(
                (tasks[l]["execution"] + slj + sumexec) / (1.0 - totalutil)
            )
            ...
            for xil in range(int(tasks[l]["period"]), int(upperbound)):
                ...
                for i in range(len(tasks)):
                    if tasks[i]["sslength"] != 0:
                        checkval += max(
                            Wnc(i, l, xil, slj, tasks), Wc(i, l, xil, slj, tasks)
                        )
                    else:
